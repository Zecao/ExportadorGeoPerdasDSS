                ...
                using (SqlCommand command = conn.CreateCommand())
                {
                    ...
                    if (_modoReconf)
                    {
                        command.CommandText = "select CodRegulMT,TnsLnh1_kV,CodFasPrim,CodPonAcopl1,CodPonAcopl2,PotNom_kVA,[ReatHL_%],"
                            + "[Resis_%],PerdVz_W,TenRgl_pu,CodBnc,TipRegul,Descr from dbo.StoredReguladorMT "
                            + "where CodBase=@codbase and CodAlim in (" + _par._conjAlim + ")";
                        command.Parameters.AddWithValue("@codbase", _par._codBase);
                    }
                    else
                    {
                        command.CommandText = "select CodRegulMT,TnsLnh1_kV,CodFasPrim,CodPonAcopl1,CodPonAcopl2,PotNom_kVA,[ReatHL_%],[Resis_%],PerdVz_W,TenRgl_pu,CodBnc,TipRegul,Descr from dbo.StoredReguladorMT where CodBase=@codbase and CodAlim=@CodAlim";
                        command.Parameters.AddWithValue("@codbase", _par._codBase);
                    ...
                    using (var rs = command.ExecuteReader())
                    {
                        ...
                        while (rs.Read())
                        {
                            // OBS: necessario converter a TnsLnh1_kV p/ fase-neutro, visto as mesmas ja virem adequadas p/ o caso do BRT em estrela // OLD CODE  
